<template>
  <div class="settings p-4 w-[500px] m-auto">
    ...
    <div class="badge settings flex items-center justify-between gap-3">
      ...
      <Switch :disabled="!canSend" :model-value="badge_expired" @update:model-value="toggleBadge" />
    </div>
  </div>
</template>
...
const toggleBadge = async () => {
  ...
  try {
    ...
    if (badge_expired.value && JSON.stringify(admin.value) != '{}') {
      const response = await $fetch(`${homeStore.baseUrl}/api/admin/allowBadge`, {
        method: "POST",
        body: {
          id: admin.value.id
        },
      ...
      const response = await $fetch(`${homeStore.baseUrl}/api/admin/expireBadge`, {
        method: "POST",
        body: {
          id: admin.value.id
        },
