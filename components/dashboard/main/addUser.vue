<template>
  <div
    class="fixed flex justify-center top-0 left-0 w-full h-full p-12 z-20 bg-white"
  >
    <form class="content relative z-10" @submit.prevent="usersStore.createUser">
      <h1 class="text-center text-3xl font-bold text-gray-700 mb-4">
        Add New User
      </h1>
      <div class="group">
        <DashboardMainInput
        ...
        <div class="el mb-4 options-wrapper">
          ...
          <div class="options flex flex-col gap-4 w-full">
            <div class="couple flex justify-between gap-4">
              <DashboardMainCheckbox
                value="exhibitor"
                group="participationTypeDash"
                link-to="participation_type"
                title="exhibitor"
                :error="usersStore.inputs.participation_type.error"
              />
              <DashboardMainCheckbox
                value="organizer"
                group="participationTypeDash"
      ...
      <div class="submit flex gap-4 items-center">
        <Button
          class="flex-1"
          :class="usersStore.canDoActions ? '' : 'cantDoActions'"
          >{{ usersStore.buttonContent.value }}</Button
        >
        <span
          class="underline cursor-pointer flex-1"
          @click="usersStore.addUser = false"
          >Close</span
        >
      </div>
    </form>
