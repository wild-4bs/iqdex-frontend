  ...
  <Popover v-model:open="open">
    <PopoverTrigger as-child>
      <Button variant="outline" role="combobox" :aria-expanded="open"
        ...
        <div v-if="value" class="flex w-full items-center justify-start gap-4">
          <img :src="props.options.find((option) => option.value === value)?.flag" class="w-[25px]" alt="">
          <span>{{props.options.find((option) => option.value === value)?.value}}</span>
        </div>
    ...
    <PopoverContent class="p-0">
      <Command>
        ...
        <CommandList>
          <CommandGroup>
            <CommandItem v-for="(option, i) in props.options" :key="i" :value="option.value" @select="(ev) => {
