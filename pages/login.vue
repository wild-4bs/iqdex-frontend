<template>
  <div class="w-full h-[100dvh] bg-gray-700 flex items-center justify-center">
    <form class="p-4 bg-white rounded-lg w-full max-w-[500px] flex flex-col gap-4" @submit.prevent="login">
      <div class="title text-center text-xl font-bold">
        <h1>Login to Malamih dashboard</h1>
      </div>
      <div class="inputs">
        <label for="emailInput">Email</label>
        <input type="email" v-model="email" required id="emailInput" placeholder="eg@gmail.com">
      </div>
      <div class="inputs">
        <label for="passInput">Password</label>
        <input type="password" v-model="password" required id="passInput" placeholder="********">
      </div>
      <div class="submit">
