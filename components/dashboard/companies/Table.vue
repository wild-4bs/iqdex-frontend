<template>
  <div
    class="edit fixed top-0 transition-all bg-white shadow-lg z-20 z p-4 bottom-0"
    ref="editor"
  ...
  >
    ...
    <div class="inputs flex flex-col gap-5">
      ...
      <div class="input">
        <NumberField
          :min="1"
          @update:model-value="(value) => (selectedCompany.users_limit = value)"
        >
  ...
  <Table class="cursor-pointer table min-w-[600px] mb-4">
    ...
    <TableBody>
      <TableRow v-for="company in companiesStore.companies" :key="company.id">
        <TableCell class="text-center capitalize">{{ company.name }}</TableCell>
        <TableCell class="text-center capitalize"
          >{{ company.users.length }} / {{ company.users_limit }}</TableCell
        >
        <TableCell class="moreOptions text-center">
          <DropdownMenu>
            ...
            <DropdownMenuContent
              :class="`${!companiesStore.canDoActions ? 'bg-gray-100' : ''}`"
            >
              <DropdownMenuItem
                @click="updateCompany(company.id)"
                :class="`${
                  !companiesStore.canDoActions ? 'cursor-not-allowed' : ''
                }`"
              >
                Edit
              </DropdownMenuItem>
              <DropdownMenuItem
                @click="companiesStore.deleteCompany(company.id)"
                :class="`${
                  !companiesStore.canDoActions ? 'cursor-not-allowed' : ''
                }`"
              >
...
const update = async () => {
  ...
  try {
    ...
    const response = await $fetch(
      `${homeStore.baseUrl}/api/company/${selectedCompany.value.id}`,
      {
        method: "PATCH",
        body: { newData: JSON.parse(dataToUpdate) },
