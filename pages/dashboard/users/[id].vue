<template>
  <div class="user">
    <div class="userExist flex flex-col xl:flex-row gap-4 p-8" v-if="JSON.stringify(user) != '{}'">
      ...
      <div class="content">
        <form @submit.prevent="submit" class="flex flex-col gap-4 h-full">
          ...
          <div class="group">
            <input type="text" v-model="user.company_name" class="item" v-if="!user.company_id">
            <DashboardUsersCompanies :id="user.company_id" @select-company="handleCompanySelect" v-else />
          ...
          <div class="group">
            <div class="item">
              <DashboardUsersParticipationTypes :type="user.participation_type" @select-type="handleTypeSelect" />
            </div>
            <div class="item">
              <DashboardUsersStatusBar :status="user.status" @select-status="handleSelect" />
            </div>
          </div>
          <div class="actions flex flex-1 gap-4  items-end">
            <Button class="flex-1" :class="canSend ? '' : ' opacity-45 cursor-not-allowed'" variant="outline">{{
              buttonContent }}</Button>
            <NuxtLink to="/dashboard"
              class="underline select-none cursor-pointer py-2 flex items-center justify-center">Cancel
            </NuxtLink>
          </div>
        </form>
      </div>
    ...
    <div class="userNotfound" v-else>
      <h1 class="text-center text-4xl text-gray-600 capitalize py-10">User not found</h1>
    </div>
  </div>
</template>
...
onMounted(async () => {
  ...
  try {
    ...
    user.value = {}
    runErrorToast({ title: "User fetching error.", message: error.statusMessage })
  }
})
