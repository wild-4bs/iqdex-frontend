<template>
  <div class="image flex gap-4">
    <label for="imageInput" :style="{ pointerEvents: inputsStore.image.value ? 'none' : '' }" ref="dropArea"
      ...
      @drop="handleDrop"
      :class="['input text-gray-500 flex flex-col relative overflow-hidden rounded-lg border-2 cursor-pointer items-center justify-between text-center border-dotted p-4 border-gray-300', dragOver ? 'dragging' : '', inputsStore.image.error ? 'error' : '']">
